...
| 模板ID | 名称 | 说明 | 关键参数 |
|--------|------|------|---------|
| 002 | 概念主题曝露度筛选 | 筛选持有指定概念板块股票的基金，双重验证（最新期+近两期中报/年报均值） | concept_codes(必填,列表,需先调get_dimension_list), min_latest_exposure(默认5%), min_hist_exposure(默认5%), fund_category_code(默认001权益基金,可选枚举), limit(默认50) |
